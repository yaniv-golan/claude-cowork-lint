...
| CW004 | `skill_frontmatter_invariants.forbidden_fields[0].field` (`disable-model-invocation`) | anchor moved (NOT removed). CLI bundle 2.1.138 has 13 `disableModelInvocation` references and 1 `skill_invoke_model_disabled` enforcement string — the field IS honored at runtime. Desktop bundle has 0 occurrences of the kebab form (manifest *display* layer is `dh(r,...)` with 4 fields: name, description, argument-hint, user-invocable), confirming the CLI runtime parser is the authoritative anchor. Task B1 should re-anchor instead of demoting. |
...
| CW010 | `user_secrets_injection.validation.{name_regex, name_max_length, reserved_name_literals}` | GONE. Desktop bundle has 0 occurrences of `OperonSecrets` and 0 of `claude.operon` (the IPC channel literal). The 3 `userConfig` references in the bundle are extension-manifest plumbing without name-validation regex. The kernel-secrets subsystem appears to have been removed from the desktop bundle in this version. CW010 needs the same demote/re-anchor treatment as CW004 — Task B4 unchanged. |
| CW011 | `cli_launch_args_in_cowork.consequences.plugin_hooks_excluded` | mechanism intact. Verified in CLI bundle: `--setting-sources=${k.join(",")}` is the actual flag formation (the SPEC's literal `--setting-sources=user` text is outdated). Plugin hooks are excluded as documented. |
